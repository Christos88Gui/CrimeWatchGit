    ...
    public class KMeansService
    {
        ...
        public void ClusterRecords()
        {
            foreach (var county in _db.Counties)
            {
                ...
                {
                    var label = labels[i];
                    records[i].CrimeRateId = _db.CrimeRates.First(x => x.Rank == label + 1).Id;
                }
            }
            _db.SaveChanges();
            CorrectRecordLabels();
        ...
        public void CorrectRecordLabels()
        {
            foreach (var county in _db.Counties)
            {
                var cluster1 = county.Records.Where(x => x.CrimeRate.Rank == 1).ToList();
                var cluster2 = county.Records.Where(x => x.CrimeRate.Rank == 2).ToList();
                var cluster3 = county.Records.Where(x => x.CrimeRate.Rank == 3).ToList();

                var records = new List<Record> { cluster1.First(), cluster2.First(), cluster3.First() };
                records = records.OrderBy(x => x.AllCrimes).ToList();

                if (cluster1.Any(x => x.Id == records.First().Id)) { foreach (var r in cluster1) { r.CrimeRateId = 32; } }
                else if (cluster1.Any(x => x.Id == records.ElementAt(1).Id)) { foreach (var r in cluster1) { r.CrimeRateId = 33; } }
                else if (cluster1.Any(x => x.Id == records.ElementAt(2).Id)) { foreach (var r in cluster1) { r.CrimeRateId = 34; } }

                if (cluster2.Any(x => x.Id == records.First().Id)) { foreach (var r in cluster2) { r.CrimeRateId = 32; } }
                else if (cluster2.Any(x => x.Id == records.ElementAt(1).Id)) { foreach (var r in cluster2) { r.CrimeRateId = 33; } }
                else if (cluster2.Any(x => x.Id == records.ElementAt(2).Id)) { foreach (var r in cluster2) { r.CrimeRateId = 34; } }

                if (cluster3.Any(x => x.Id == records.First().Id)) { foreach (var r in cluster3) { r.CrimeRateId = 32; } }
                else if (cluster3.Any(x => x.Id == records.ElementAt(1).Id)) { foreach (var r in cluster3) { r.CrimeRateId = 33; } }
                else if (cluster3.Any(x => x.Id == records.ElementAt(2).Id)) { foreach (var r in cluster3) { r.CrimeRateId = 34; } }
            }
            _db.SaveChanges();
        }
